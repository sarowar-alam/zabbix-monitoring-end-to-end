#!/bin/bash
# =============================================================================
# Zabbix 7.0 LTS Agent Installation — Linux via Proxy
# =============================================================================
# Run as root on the linux-agent-proxy EC2 instance (private subnet).
#
# Usage:
#   sudo bash install-agent-linux-proxy.sh <PROXY_PRIVATE_IP>
#
# Example:
#   sudo bash install-agent-linux-proxy.sh "10.0.2.x"
#
# This agent:
#   - Runs in ACTIVE mode  (agent dials OUT to proxy on port 10051)
#   - Reports all metrics THROUGH the Zabbix Proxy
#   - No inbound port 10050 is needed on this instance's security group
#
# After running this script:
#   1. Register this host in the Zabbix UI (see INSTALL.md Phase 3)
#   2. Copy task-linux.sh from 06-zabbix-sender/ for scheduled task alerting
# =============================================================================
...
PROXY_IP="${1:?ERROR: Proxy private IP required. Usage: sudo bash $0 <PROXY_PRIVATE_IP>}"
THIS_HOSTNAME="linux-agent-proxy"
ZABBIX_VERSION="7.0"
...
echo "============================================================"
echo "  Zabbix ${ZABBIX_VERSION} Agent Installation (via Proxy)"
...
# Server = proxy IP (used for passive checks; also serves as allow-list for active)
apply_conf "Server"              "${PROXY_IP}"      "${AGENT_CONF}"
# ServerActive = proxy IP:port (agent dials OUT here to deliver metrics actively)
apply_conf "ServerActive"        "${PROXY_IP}"      "${AGENT_CONF}"
...
# Include drop-in config dir (for custom UserParameters from task wrappers)
...
echo ""
echo "  Proxy IP : ${PROXY_IP}:10051  (active connection)"
echo "  Hostname : ${THIS_HOSTNAME}"
echo ""
echo "  IMPORTANT — Add this host in the Zabbix UI (see INSTALL.md Phase 3):"
echo "    Configuration → Hosts → Create host"
echo "    Host name : ${THIS_HOSTNAME}"
echo "    Monitored by proxy : ZabbixProxy01"
